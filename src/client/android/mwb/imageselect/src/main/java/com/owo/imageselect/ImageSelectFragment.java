...
public class ImageSelectFragment extends Fragment {
    ...
    private void selectImageFromGrid(Image image, int mode) {
        if (image != null) {
            if (mode == MODE_MULTI) {
                if (resultList.contains(image.getUrl())) {
                    resultList.remove(image.getUrl());
                    if (mCallback != null) {
                        mCallback.onImageUnselected(image.getUrl());
                    }
                } else {
                    ...
                    resultList.add(image.getUrl());
                    if (mCallback != null) {
                        mCallback.onImageSelected(image.getUrl());
                    }
                }
                mImageAdapter.select(image);
            } else if (mode == MODE_SINGLE) {
                if (mCallback != null) {
                    mCallback.onSingleImageSelected(image.getUrl());
                }
            }
        }
    }
